...
function convertNumber2Currency($num)
{
  list($num, $dec) = explode(".", $num);
  $output = "";
  ...
  if($dec > 0)
  {
    $output .= " and";
      if(strlen($dec) == 1){
        $output .= " ".convertTwoDigit($dec[0], 0);
      }else{
        if($dec[0] == 0){
          $output .= " ".convertDigit($dec[1]);
        }else{
...
//conver the negative symbol to parentheses. ie -123 => (123)
function neg2parentheses($value){
